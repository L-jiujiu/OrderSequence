...
class Order:
    def __init__(self, order_config):
        ...
        # 剩余分区集合，表示当前分区结束后要去的分区（按顺序从0,1,...）；
        self.order_section_list = order_config['order_section_list']
        # 若为空集，表示订单在完成当前分区分拣后，就完成了全部分拣作业
        # 简单的分区经过信息，不同sku在同一分区只记录一次
        self.order_section_list_simple = order_config['order_section_list_simple']
        # 新增状态
        # 当前所在分区：若非空，则表示已经在分区中等待或分拣
        self.current_section = order_config['current_section']
        # 时间
